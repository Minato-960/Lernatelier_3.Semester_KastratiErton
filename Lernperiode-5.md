# Lern-Periode 5
- Name: Kastrati Erton
- Zeitraum: 14.08.2026 bis 25.09.2026

## Grob-Planung
Ich will Java und C# wieder erlernen und ausbessern. Meinen Laptop Dual-Booten und Linux erlernen.
### Noten
> Wo stehen Sie mit Ihren Noten? In welchen Modulen waren Sie besonders stark; in welchen sind die ungenügend? Welche davon sind besonders wichtig?
Ich stehe gerade nirgendwo mit den Noten doch welche wichtig sein werden sind eigentlich alle. 320 wird ein sehr wichtiges Modul sein.
### Veränderungen
> Was möchten Sie generell im Vergleich zur letzten Lernperiode anpassen?
Die Lernperiode aktiv mitzuführen und die Einträge Seriös nehmen.
### Projekte / neue Technologien
> Was für Projekte/neue Technologien möchten Sie gerne in dieser Lernperiode lernen?
Ich will am Minecraft Casino Mod weiterarbeiten
### Generelle Ziele
> Was haben Sie für klare und messbare Ziele in dieser Lernperiode?
- Java lernen
- C# wiedererlernen
## Tagesplanungen

### Planung 14.08.2026
3 bis 5 klar messbare Arbeitspakete.

- [X] Lernperiode erstellt
- [X] Das Konzept meines Java Projekts wieder anschauen und meinen Standort bestimmen
- [ ] ...

(Heute habe ich die Lernperiode erstellt und meinen stand im Java Projekt wieder angeschaut.)

### Planung 21.08.2026
3 bis 5 klar messbare Arbeitspakete.

- [X] C++ Einführung beenden
- [X] Mich über Partition verkleinerung informieren.
- [X] Mich über Dual-Boot informieren und es in einer VM ausprobieren.

(Heute habe ich mich bei Cisco Networking Academy angemeldet und angefangen das C++ zu bearbeiten. Und eine VM mit einem Dual-Boot System eingerichtet.)


### Planung 28.08.2026
3 bis 5 klar messbare Arbeitspakete.

- [X] Meinen Laptop Dual-Booten (CachyOS installieren)
- [X] C++ Modul 1 Aufgabe 1.3 und 1.4 bearbeitet.
- [X] Mein Linux System eingerichtet fürs zukünftige arbeiten.

(Heute habe ich die Aufgaben 1.3 und 1.4 im Cisco Networking Academy bearbeitet und auf meinem Laptop CachyOS eingerichtet da ich diese Woche noch keine Zeit dafür gefunden habe.)

### Planung 04.09.2026
3 bis 5 klar messbare Arbeitspakete.

- [ ] C++ Modul 1 beenden.
- [ ] C++ Modul 2 Aufgabe 2.0 und 2.1 bearbeiten.
- [ ] Hyprland Dotfiles verstehen.

(Heute habe Ich...)

## Lernperiode Reflexion
(In dieser Lernperiode habe ich... (100-150 Wörter))
